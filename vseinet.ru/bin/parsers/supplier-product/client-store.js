...
class Store {
    ...
    setRequestData(source, data) {
        const insert = `
            INSERT IGNORE INTO data_supplier_product (
                id, 
                supplier_product_id, 
                source_id, 
                name, 
    ...
    setResultData(source, data, result) {
        if (result.data) {
            ...
            data.data = extend(true, data.data, result.data);
    ...
    cleanup(ids) {
        return this.mysql.query(`
            DELETE FROM data_supplier_product 
            WHERE id IN (:ids)`, 
            { ids }
        );
    }
