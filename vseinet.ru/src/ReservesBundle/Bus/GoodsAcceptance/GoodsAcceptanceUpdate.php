...
class GoodsAcceptanceUpdate
{

    /**
     * Отмена регистрации документа в учетной системе.
     *
     * @param \ReservesBundle\Entity\GoodsAcceptance $newDocument регистрируемый документ
     * @param \ReservesBundle\Entity\GoodsAcceptance $oldDocument старый регистрируемый документ
     ...
     */
    public static function Update(GoodsAcceptance $newDocument, GoodsAcceptance $oldDocument, \Doctrine\ORM\entityManager $em, \AppBundle\Entity\User $currentUser)
    {
        ...
        if ($newDocument->getStatusCode() != $oldDocument->getStatusCode()) {
            ...
            $statusQeryText = "
                ...
                from goods_acceptance_doc_status
                where
