...
use AppBundle\Bus\Catalog\Query\DTO\Filter\CategorySection;

class CategorySections
{
    /**
     * @return array<CategorySection>
     */
    public static function build(array $section2count, EntityManager $em): array 
    {
        if (!empty($section2count)) {
            $q = $em->createQuery("
                SELECT 
                    NEW AppBundle\Bus\Catalog\Query\DTO\Filter\CategorySection (
                        cs.id, 
                        cs.name 
                    )
                FROM AppBundle:CategorySection cs 
                WHERE cs.id IN (:ids) 
            ");
