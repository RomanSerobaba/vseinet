...
class QueryBuilder extends ContainerAware
{
    ...
    public function getProducts($isSearch = false): array
    {
        ...
        $options = 'ranker=expr(\'sum(if(min_best_span_pos < 5, 5 - min_best_span_pos, 0) + exact_order * 10) + if(availability < 4, 4 - availability, 0) * 2 + (1 - is_accessories) * 10 + if(category_average_price > 100000, 5, 0) + if(popularity > 50, 10, 0) + if(name_length < 120, 10, 0)\'), max_matches='.self::MAX_MATCHES;
