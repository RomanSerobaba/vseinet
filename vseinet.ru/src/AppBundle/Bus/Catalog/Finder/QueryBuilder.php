...
class QueryBuilder extends ContainerAware
{
    ...
    public function getProducts($isSearch = false): array
    {
        ...
        if (Sort::PRICE === $filter->sort) {
            ...
            $sort = $isSearch ? 'weight DESC, availability ASC, rating DESC, price ASC' : 'availability ASC, popularity DESC, price ASC';
        }

        $page = min($filter->page, ceil(self::MAX_MATCHES / self::PER_PAGE));
        $offset = ($page - 1) * self::PER_PAGE;

        $options = 'ranker=expr(\'sum(if(min_best_span_pos < 5, 5 - min_best_span_pos, 0) + exact_order * 15) + if(availability = 1, 12, if(availability < 4, 4 - availability, 0) * 2) + (1 - is_accessories) * 15 + if(category_average_price > 100000, 12, 0) + if(popularity > 500, 10, popularity / 50) + if(name_length < 120, 10, 0)\'), max_matches='.self::MAX_MATCHES;
