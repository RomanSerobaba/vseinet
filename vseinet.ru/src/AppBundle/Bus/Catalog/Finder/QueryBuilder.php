...
class QueryBuilder extends ContainerAware
{
    ...
    public function getProducts($isSearch = false): array
    {
        ...
        $options = 'ranker=expr(\'sum(IF(5 - min_best_span_pos > 0, 5 - min_best_span_pos, 0) * 5 + lcs * 5 + exact_hit * 5) + if(availability < 4, 1, 0) * 20 +  IF(price < 2000, 0, IF(price < 10000, 1, 2)) * 3\'), max_matches='.self::MAX_MATCHES;
