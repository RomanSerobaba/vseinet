...
class AutocompleteFinder extends AbstractProductFinder
{
    ...
    public function getResult()
    {
        ...
        $query = "
            ...
            OPTION ranker=expr('sum(if(min_best_span_pos < 5, 5 - min_best_span_pos, 0) + exact_order * 15) + if(availability = 1, 12, if(availability < 4, 4 - availability, 0) * 2) + (1 - is_accessories) * 15 + if(category_average_price > 100000, 12, 0) + if(popularity > 500, 10, popularity / 50) + if(name_length < 120, 10, 0)')
            ;
        ";
        $results = $this->get('sphinx')->createQuery()->setQuery($query)->getResults();
        if (!empty($results[0])) {
