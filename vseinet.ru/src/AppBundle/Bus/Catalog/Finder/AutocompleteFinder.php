...
class AutocompleteFinder extends AbstractProductFinder
{
    ...
    public function getResult()
    {
        ...
        $query = "
            ...
            OPTION ranker=expr('sum(exact_hit * 15 + if(min_best_span_pos < 5, 5 - min_best_span_pos, 0) + exact_order * 10) + if(availability < 4, 4 - availability, 0) * 2 + (1 - is_accessories) * 5 + if(category_average_price > 500000, 5, 0) + if(popularity > 50, 10, 0) + if(name_length < 120, 10, 0)')
            ;
        ";
        $results = $this->get('sphinx')->createQuery()->setQuery($query)->getResults();
        if (!empty($results[0])) {
