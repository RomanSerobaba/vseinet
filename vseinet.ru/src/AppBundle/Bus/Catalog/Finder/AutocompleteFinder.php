...
class AutocompleteFinder extends AbstractProductFinder
{
    ...
    public function getResult()
    {
        ...
        $query = "
            ...
            OPTION ranker=expr('sum(sum_idf * 100 + if(min_best_span_pos < 5, 5, 0)) + if(availability < 4, 4 - availability, 0) * 4 + if(category_average_price > 500000, 2, if(category_average_price > 80000, 1, 0)) * 10 + if(popularity > 50, 10, 0)')
            ;
        ";
        $results = $this->get('sphinx')->createQuery()->setQuery($query)->getResults();
        if (!empty($results[0])) {
