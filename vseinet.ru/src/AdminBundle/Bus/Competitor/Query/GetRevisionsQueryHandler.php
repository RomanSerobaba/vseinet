    ...
    public function handle(GetRevisionsQuery $query)
    {
        ...
        $q = $em->createNativeQuery("
            SELECT
                ...
                cp.requested_at,
                cp.status,
                CASE WHEN COALESCE(cp.price, 0) = 0 OR c.period IS NOT NULL AND cp.completed_at + (c.period || ' day')::INTERVAL < NOW() THEN :void WHEN cp.price > COALESCE(p2.price, p.price) THEN :ice ELSE :warning END AS state,
            ...
            WHERE bp.canonical_id = :base_product_id AND cp.geo_city_id IN (:geo_city_id, 0) AND c.is_active = true
