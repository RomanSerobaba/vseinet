    ...
    public function handle(SearchQuery $query)
    {
        ...
        $q = $em->createQuery("
            SELECT
                ...
                CASE WHEN LOWER(c.name) LIKE LOWER(:qo) THEN 1 ELSE 2 END AS HIDDEN ORD
            ...
            WHERE c.aliasForId IS NULL
                ...
                AND (LOWER(c.name) LIKE LOWER(:q1) OR LOWER(c.name) LIKE LOWER(:q2))
            ORDER BY ORD, c.name
        ");
