...
class BankOperationDocUpdate
{

    /**
     * Обновление документа
     *
     * @param \FinanseBundle\Entity\BankOperationDoc  $newDocument  регистрируемый документ
     * @param \FinanseBundle\Entity\BankOperationDoc  $oldDocument  старый регистрируемый документ
     ...
     */
    public static function Update(BankOperationDoc $newDocument, BankOperationDoc $oldDocument, \Doctrine\ORM\entityManager $em, \AppBundle\Entity\User $currentUser)
    {
        ...
        if ($newDocument->getStatusCode() != $oldDocument->getStatusCode()) {
            ...
            $statusQeryText = "
                ...
                from bank_operation_doc_status
                where
