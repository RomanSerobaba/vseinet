...
class GetContactsQueryHandler extends MessageHandler
{
    public function handle(GetContactsQuery $query)
    {
        ...
        $q = $em->createQuery("
            ...
            INNER JOIN GeoBundle:GeoCity gc WITH gc.id = gp.geoCityId 
            INNER JOIN GeoBundle:GeoRegion gr WITH gr.id = gc.geoRegionId
        ...
        $q = $em->createQuery("
            SELECT 
                NEW SiteBundle\Bus\Representative\Query\DTO\Phone (
                    rp.representativeId,
                    c.value 
                ),
        ...
        $q = $em->createQuery("
            SELECT 
                NEW SiteBundle\Representative\Query\DTO\Schedule (
                    rs.representativeId,
                    rs.s1, rs.t1,
                    rs.s2, rs.t2,
                    rs.s3, rs.t3,
