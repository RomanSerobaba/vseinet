...
use SiteBundle\Bus\Catalog\Query\DTO\Category;
use SiteBundle\Bus\Brand\Query\DTO\Brand;
...
class CategoryProductFinder extends ProductFinder
{
    ...
    public function getFilter(): Filter 
    {
        ...
        if ($this->category->isTplEnabled) {
            $q = $em->createQuery("
                SELECT 
                    NEW SiteBundle\Bus\Catalog\Query\DTO\Filter\Detail (
                        d.id,
                        d.name,
        ...
        if (!empty($valueIds)) {
            $q = $em->createQuery("
                SELECT
                    NEW SiteBundle\Bus\Catalog\Query\DTO\Filter\DetailValue (
                        dv.id,
                        dv.value 
                    )
        ...
        foreach ($this->filter->details as $id => $detail) {
            if (isset($this->filter->sections[$detail->sectionId])) { // @hack
