...
-- SiteBundle\Bus\Catalog\Query\Enum\Availability.php
...
SELECT 
    p.base_product_id,
    json_object_agg(COALESCE(p.geo_city_id, 0),
        CASE 
    ...
    json_object_agg(COALESCE(p.geo_city_id, 0), ROUND(p.price / 100)) AS price, -- in rubles
    json_object_agg(COALESCE(p.geo_city_id, 0), 
        CASE 
    ...
    json_object_agg(COALESCE(p.geo_city_id, 0), p.profit) AS profit, -- in pennies
    json_object_agg(COALESCE(p.geo_city_id, 0), p.rating) AS rating
FROM product AS p
...
-- SiteBundle\Bus\Catalog\Query\Enum\Nofilled.php
